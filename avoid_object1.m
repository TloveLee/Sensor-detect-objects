%% Assignment2 
%% =================
%% This function gets the object value of sensor value as input, in the first moving path,
...
%% ------------------------------------------------------------
function [ Sx,Sy] = avoid_object1( xmin,xmax,ymin,ymax,cxmin,cxmax,cymin,cymax ,objectx,circlex,Sx,Sy,t1,s)
for num1 = 1:length(objectx) %using for loop to detect every rectangles and suqares
      % Determine whether the sensor is left or right to avoid the objects
        if Sx{s}(t1) >= xmin(num1) & Sx{s}(t1) <= xmax(num1) & Sy{s}(t1) >= ymin(num1) & Sy{s}(t1) <= ymax(num1) 
            if Sx{s}(t1) -xmin(num1) < xmax(num1) - Sx{s}(t1)
            Sx{s}(t1) = xmax(num1) + 1;
            else
            Sx{s}(t1) = xmin(num1) -1;
            end
...
for num2 = 1:length(circlex) %using for loop to detect every circles
    % Determine whether the sensor is left or right to avoid the objects
          if Sx{s}(t1) >= cxmin(num2) & Sx{s}(t1) <= cxmax(num2) & Sy{s}(t1) >= cymin(num2) & Sy{s}(t1) <= cymax(num2) 
            if Sx{s}(t1) -cxmin(num2) < cxmax(num2) - Sx{s}(t1)
            Sx{s}(t1) = cxmax(num2) + 1;
            else
            Sx{s}(t1) = cxmin(num2) -1;
            end
        end
end
end
